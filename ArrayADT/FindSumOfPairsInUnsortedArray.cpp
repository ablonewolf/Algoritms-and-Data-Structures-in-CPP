...
void printPair(int *array, int size, int expectedSum, int maxElement) {
    ...
    for (int i = 0; i < size; i++) {
        if (array[i] <= expectedSum && countArray[array[i]] >
                                       0) // only process that element which is less than expectedSum and has not been processed yet
        {
            int k = expectedSum - array[i];
            if (k == array[i] && countArray[k] >
                                 1) // if the subtraction equals to the element itself, it means we need two of it to equal to the sum, so looking for the duplicate
            {
                count++;
            ...
            } else if (countArray[k] > 0 && array[i] !=
                                            k) // if the subtracted value exists in the array and it is not equal to the first found element
            {
                count++;
                cout << "Pair found. One element is: " << array[i] << ", another element is: " << k << endl;
